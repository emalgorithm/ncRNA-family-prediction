...
class RNAFamilyGraphDataset1(InMemoryDataset):
    def __init__(self, file_path, foldings_path, transform=None, pre_transform=None,
                 seq_max_len=10000,
                 seq_min_len=1, n_samples=None):

        print(' File: "' + __file__ + '", Line ' + str(
            sys._getframe().f_lineno) + ' , in ' + sys._getframe().f_code.co_name)

        super(RNAFamilyGraphDataset1, self).__init__(file_path, transform, pre_transform)

        print(' File: "' + __file__ + '", Line ' + str(
            sys._getframe().f_lineno) + ' , in ' + sys._getframe().f_code.co_name)

        with open(file_path+'/val.fasta', "r") as handle:
